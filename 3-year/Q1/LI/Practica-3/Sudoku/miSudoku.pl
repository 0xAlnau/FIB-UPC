...
squareOfBlock( Iid,Jid, I,J ) :- row(I), col(J), Iid is (I-1) // 3,  Jid is (J-1) // 3. % relaciona pos normal amb blockID
...
satVariable( x(I,J,K) ) :- row(I), col(J), val(K). %en coord i,j tenim valor k


%%%%%%%  2. Clause generation for the SAT solver: =============================================

writeClauses :-                %Posar les restriccions!!!!!
...
filledInputValues :-
        entrada(Sud),
        nth1(I,Sud,Row), nth1(J,Row,K), %en la pos i,j tenim el valor k
        integer(K), %comprova es enter
        writeOneClause([ x(I,J,K) ]), %afegeix la clausula
        fail. %backtracking per generar totes les clausules possibles, fa TOT el sudoku :)
filledInputValues.
