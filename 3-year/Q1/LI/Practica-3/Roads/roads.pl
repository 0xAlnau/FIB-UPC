...
%% maxStations(14).
%%
%% road(18,19). % conecta ciutat 18 amb la 19
%% road(6,18).
%% road(1,12).
%% road(11,19).
%% road(3,15).
...
writeClauses :-
    %% Complete this!
    oneGasStationBetweenRoads, %entre pobles un gas station, generar clausules
    belowLimitStations, %comprovacio
    true,!.
writeClauses :- told, nl, write('writeClauses failed!'), nl, nl, halt.

oneGasStationBetweenRoads :-
        road(C1,C2), %comprovar quines dues ciutats estan conectades
        writeOneClause([ install(C1) , install(C2)]), %afegeix nova clausula, es faran les 4 combis
        fail.
oneGasStationBetweenRoads.

belowLimitStations :- % cal findall
        findall(install(C), city(C), Lits), %dintre de les instalats mira quantes hi ha amb city C
        maxStations(MS),
        atMost(MS, Lits), %com a molt MS amb gasolina
        fail.
belowLimitStations.
