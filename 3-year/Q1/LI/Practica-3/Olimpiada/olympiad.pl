...
realFriends :-
    ...
    S1 \= S2,
    \+ friends(S1,S2),          % si S1 no el considera amic a S2
    writeOneClause([-chosen(S1), -chosen(S2)]),  % no son amics, no els podem agafar
    fail.
realFriends.


differentExpertise :-
    expertise(S1, E), expertise(S2, E),
    S1 \= S2,
    writeOneClause([-chosen(S1), -chosen(S2)]),  % mateixa expertesa, no els podem agafar
    fail.
differentExpertise.
