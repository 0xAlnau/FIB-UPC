...
mes_gros(elefant,cavall).
mes_gros(cavall,ruc).
mes_gros(ruc,gos).
mes_gros(ruc,gat).
mGros(X,Y) :- mes_gros(X,Y).
mGros(X,Y) :- mes_gros(X,Z), mGros(Z,Y).
...
% subcjt(S,T): S és un subconjunt de T (permet generar subconjunts S de T)
subcjt([],[]).
subcjt(S,[_|L]):- 
	subcjt(S,L).
subcjt([X|S],[X|L]):- 
...
% mida(L,N): el nombre d'elements de L és N
% (length/2)   
mida([],0).
