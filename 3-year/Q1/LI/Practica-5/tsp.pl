...
lowerBoundOfRemainingCities([City | RemCities], LBound) :- %agafarem el vei amb cost mínim de cada ciutat que queda
	findall(K, (distance(City,_,K)), Distancies), %llista de distancies desde City
	min_list(Distancies, MinDist), %agafem la minima
	lowerBoundOfRemainingCities(RemCities, RemLBound),
	LBound is RemLBound + MinDist.

myselect( CurrentCity, NextCity, CandidateCities, RemainingCities ) :-
    findall(D-C, (member(C, CandidateCities), distance(CurrentCity, C, D)), DistList), %creem llista per parells, Dist-Ciutat
    keysort(DistList, SortedDistList), %ordenem per dist
    member(_-NextCity, SortedDistList), %agafa la ciuat amb menys dist
    select(NextCity, CandidateCities, RemainingCities). %creem nova llista amb ciutats restants
...
tsp( Cities, AccumulatedKm, _ ):-
	bestRouteSoFar(Km,_),
	LBound is 0,
    lowerBoundOfRemainingCities( Cities, LBound ),   %implement this (efficiently)! lowerBound es el que queda de Km per fer!
    AccumulatedKm+LBound >= Km, !, fail. %si el que ja he fet mes el que hem queda supera el que tenia, no segueixo!

%3
tsp( Cities, AccumulatedKm, [ CurrentCity | RouteSoFar ] ):-
%   select( City, Cities, RemainingCities ),  % select next city to visit
    myselect( CurrentCity, City, Cities, RemainingCities ),  % select next city to visit (implement this)
