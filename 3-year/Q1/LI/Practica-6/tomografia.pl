...
%2: Constraints:
        matrixByRows(L, NumCols, MatrixByRows), % crea la matriu
        transpose(MatrixByRows, MatrixByColumns), % creem la transposada per despres fer la suma de columnes millor!
...
% extra
matrixByRows([],_,[]) :- !.
matrixByRows(L, NumCols, [X | M]) :- % anem encadenant llistes
        length(X,NumCols),
        append(X,L1,L), %X es la fila
        matrixByRows(L1,NumCols,M).


declareConstraints(RowSums,ColSums,MatrixByRows,MatrixByColumns) :-
        checkSum(RowSums,MatrixByRows),
        checkSum(ColSums,MatrixByColumns).
