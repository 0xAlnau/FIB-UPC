...
writeClauses(MaxNumTowers):-
    oneTowerInVillage(),
    noTowersOutsideAVillage(),
    villagesWatched(),
    importantHasTower(),
    maximumTowers(MaxNumTowers),
    novaVariable(),
...
importantHasTower() :-
    ...
    writeOneClause([towerInVillage(V)]), %ja hem comprovat abans que tingui tower
    fail.
importantHasTower().
...
maximumTowers(MaxNumTowers) :-
    ...
    findall(towerInVillage(V), village(V), Lits),
    atMost(Max, Lits),
    fail.
maximumTowers(_).
