...
satVariable( active(T,H) ) :- task(T), hour(H). % "task T is active at hour H"
...
writeClauses(MaxHours) :-
    ...
    eachTaskActiveOncePerHour(MaxHours),
    eachResourceUsedWisely(MaxHours),
    true, !.
...
eachTaskStartsOnce(MaxHours) :-
    task(T),
    duration(T,D),
    HMax is MaxHours - D + 1,
    findall(start(T,H), between(1, HMax, H), Lits),
    exactly(1, Lits),
    fail.
...
eachTaskActiveOncePerHour(MaxHours) :-
    ...
    Hx =< M,
    Hx =< MaxHours,
    writeOneClause([-start(T,H), active(T,Hx)]),
    fail.
eachTaskActiveOncePerHour(_).
...
eachResourceUsedWisely(MaxHours) :-
   ...
   maxHour(M),
   H =< M,
   resourceUnits(R, Unitats),
   findall(active(T,H), usesResource(T,R), Lits),
   atMost(Unitats, Lits),
   fail.
eachResourceUsedWisely(_).
...
costOfThisSolution(M,Cost) :-
    ...
    findall(End, ( member(start(T,H),M), duration(T,D), End is H+D-1), L),  max_list(L,Cost),!.
