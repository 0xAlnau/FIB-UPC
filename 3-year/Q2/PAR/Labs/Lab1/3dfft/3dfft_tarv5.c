...
void init_complex_grid(fftwf_complex in_fftw[][N][N]) {
  ...
  for (k = 0; k < N; k++) {
  tareador_start_task("init_complex_grid_loopk");
    for (j = 0; j < N; j++) {
    tareador_start_task("init_complex_grid_loopj");
      for (i = 0; i < N; i++)
      {
    ...
    tareador_end_task("init_complex_grid_loopj");
    }
  tareador_end_task("init_complex_grid_loopk");
  }
}
...
void transpose_xy_planes(fftwf_complex  tmp_fftw[][N][N], fftwf_complex in_fftw[][N][N]) {
    ...
    for (k=0; k<N; k++) {
     tareador_start_task("xy_planes_loop_k");
     for (j=0; j<N; j++) {
       tareador_start_task("xy_planes_loop_j");
       for (i=0; i<N; i++)
       {
         tmp_fftw[k][i][j][0] = in_fftw[k][j][i][0];
         tmp_fftw[k][i][j][1] = in_fftw[k][j][i][1];
       }
       tareador_end_task("xy_planes_loop_j");
     }
     tareador_end_task("xy_planes_loop_k");
    }
}
...
void transpose_zx_planes(fftwf_complex in_fftw[][N][N], fftwf_complex tmp_fftw[][N][N]) {
    ...
    for (k=0; k<N; k++) {
     tareador_start_task("zx_planes_loop_k");
     for (j=0; j<N; j++) {
     tareador_start_task("zx_planes_loop_j");
       for (i=0; i<N; i++)
       {
         in_fftw[i][j][k][0] = tmp_fftw[k][j][i][0];
         in_fftw[i][j][k][1] = tmp_fftw[k][j][i][1];
       }
     tareador_end_task("zx_planes_loop_j"); 
     }
     tareador_end_task("zx_planes_loop_k");
    }
}
...
int main (int argc, char *argv[]) {
    ...
    tareador_start_task("planes1");
    ffts1_planes(p1d, in_fftw);
    tareador_end_task("planes1");

    tareador_start_task("xy_planes1");
    transpose_xy_planes(tmp_fftw, in_fftw);
    tareador_end_task("xy_planes1");
    
    tareador_start_task("planes2");
    ffts1_planes(p1d, tmp_fftw);
    tareador_end_task("planes2");

    tareador_start_task("zx_planes");
    transpose_zx_planes(in_fftw, tmp_fftw);
    tareador_end_task("zx_planes");

    tareador_start_task("planes3");
    ffts1_planes(p1d, in_fftw);
    tareador_end_task("planes3");

    tareador_start_task("zx_planes2");
    transpose_zx_planes(tmp_fftw, in_fftw);
    tareador_end_task("zx_planes2");

    tareador_start_task("xy_planes2");
    transpose_xy_planes(in_fftw, tmp_fftw);
    tareador_end_task("xy_planes2");
