...
/* Q1: Why all tasks are created and executed by the same thread?     */ //no es paralel perque només es fa una tasca i ja està, el 0 la crea i com ja està despert la torna a fer
/*     In other words, why the program is not executing in parallel?  */
/* Q2: Modify the code so that tasks are executed in parallel and     */ //fet
/*     each iteration of the while loop is executed only once         */
/* Q3: What is the firstprivate(p) clause doing? Comment it and       */ //fa que el valor de p es
	//es guardi i no doni segmentation fault
