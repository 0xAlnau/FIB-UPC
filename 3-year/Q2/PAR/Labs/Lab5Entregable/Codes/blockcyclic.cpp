...
#ifndef ROWS
#define ROWS (512*10)             /* size of display window in pxiels */
#endif

#ifndef COLS
#define COLS (512*10)             /* size of display window in pixels */
#endif
...
void mandel_simple(int M[ROWS][COLS], double CminR, double CminI, double CmaxR, double CmaxI, double scale_real, double scale_imag, int maxiter)
{
	#pragma omp parallel
	{
      ...
      int BS = 64/sizeof(int);
      
      for (int y = 0; y < ROWS; y++) {
	    for (int xx = my_id*BS; xx < COLS; xx+=howmany*BS) {
	    	for (int x = xx ; x < xx +BS && x < COLS;x++)
	    	{ 
...
#define HUGE_PAGE_SIZE (1024 * 2 * 1024)
...
int main(int argc, char *argv[])
{
      ...
      int height = ROWS;
      double size = N, x0 = 0.1001, y0 = 0.1001;
      int *Hmatrix;
      unsigned char *image;
