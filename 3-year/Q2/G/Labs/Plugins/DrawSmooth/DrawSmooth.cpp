...
bool Draw_smooth::drawObject(int i)
{
    ...
    if (useIndices)
    {
        ...
        g.glDrawElements(GL_TRIANGLES, numElements[i], GL_UNSIGNED_INT, (void*)0);
...
void Draw_smooth::addVBO(unsigned int currentObject)
{
    ...
    if (useIndices)
    {
        ...
        for (unsigned int i = 0; i < verts.size(); i++)
        {
            ...
            // En modo suave, cada vértice tiene su propia normal única
            Vector N = Ns[i];
            normals.push_back(N.x());
            normals.push_back(N.y());
            normals.push_back(N.z());

            // El color en GLarena suele basarse en el valor absoluto de la normal
            colors.push_back(fabs(N.x()));
            colors.push_back(fabs(N.y()));
            colors.push_back(fabs(N.z()));

            // Coordenadas de textura (protección por si el modelo no tiene)
            pair<float, float> TC = texcords.empty() ? make_pair(0.0f, 0.0f) : texcords[i];
    ...
    else
    {
        ...
        for (auto&& f: obj.faces()) 
        {
            ...
            // 2. Calculamos la media para obtener la normal plana de la cara
            Vector faceNormal = (n0 + n1 + n2) / 3.0f;
