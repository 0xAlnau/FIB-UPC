 ...
 */
#define PLAYER_NAME Astornauta7
...
struct PLAYER_NAME : public Player {
   ...
   *///                          0    1      2    3     4   5   6   7
   ...
   bool in_group(Pos p) {
     int count = 0;
     for (int i = p.i-2; count < 4 and i <= p.i+2; ++i) {
        for (int j = p.j-4; count < 4 and j <= p.j+4; ++j) {
            if (pos_ok(Pos(i,j)) and cell(Pos(i,j)).id != -1 and cell(Pos(i,j)).owner == me()) ++count;
        }
     }
   ...
   pair<int,Dir> found_group(Pos p) {
     bool found = false;
     pair<int,Dir> resultat;
     resultat.first = INF;
     ...
     while (not Q.empty() and not found) {
       ...
       for (int i = 0; not found and i < 4; ++i) {
         ...
         if (pos_ok(u) and cell(u).type != Wall and distances[u.i][u.j] == INF) {
           ...
           distances[u.i][u.j] = distances[v.i][v.j] + 1;
           previ[u.i][u.j] = dirs_mag[i];
           if (cell(u).id != -1 and cell(u).owner == me() and in_group(unit(cell(u).id).pos) and u != pos_voldemort()) {
             found = true;
             resultat.first = distances[u.i][u.j];
             resultat.second = cami_invers_m(p, u, previ);
           }
     ...
     while (not Q.empty() and not found) {
       ...
       for (int i = 0; not found and i < 4; ++i) {
         ...
         if (pos_ok(u) and (cell(u).id == -1 or (cell(u).id != -1 and unit(cell(u).id).player != me())) and cell(u).type != Wall and distances[u.i][u.j] == INF) {
           ...
           distances[u.i][u.j] = distances[v.i][v.j] + 1;
           previ[u.i][u.j] = dirs_mag[i];
           if (cell(u).owner != me() and cell(u).owner != -1 and cell(u).id != -1 and u != pos_voldemort() and unit(cell(u).id).type != Ghost and not unit(cell(u).id).is_in_conversion_process() and not (2 * magic_strength(me()) < magic_strength(cell(u).owner))) {
             found = true;
             resultat.first = distances[u.i][u.j];
             resultat.second = cami_invers_m(p, u, previ);
           }
   ...
   virtual void play () {
     ...
     for (int id_wiz : ids_mags) {
       ...
       if (unit(id_wiz).is_in_conversion_process()) { //que vagi a per ajuda
         ...
         if (round() <= num_rounds() -60) { //mode recolecta i agrupacio
