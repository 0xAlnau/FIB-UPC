 ...
 */
#define PLAYER_NAME Astornauta3
...
struct PLAYER_NAME : public Player {
  ...
  int quadrant(Pos p) {
    int resultat;
    if (p.i <= 29) {
      if (p.j <= 29) resultat = 1;
      else resultat = 2;
    }
    else {
      if (p.j <= 29) resultat = 3;
      else resultat = 4;
    }
    return resultat;
  }
  ...
  Pos look_no_wall(Pos p) {
      if (cell(p).type != Wall) return p;
      else {
        for (int i = 0; i < 8; ++i) {
          Pos u = p + dirs_gho[i];
          if (cell(u).type != Wall) return u;
        }
      }
      return p;
  }

  pair<int,Dir> go_to(Pos ini, Pos fin) {
    bool found = false;
    pair<int,Dir> resultat;
    resultat.first = INF;
    resultat.second = Left;
      ...
      for (int i = 0; not found and i < 4; ++i) {
        ...
        if (pos_ok(u) and cell(u).type != Wall and distances[u.i][u.j] == INF) {
          ...
          if (u == fin) {
            ...
            resultat.second = cami_invers_m(ini, u, previ);
          }
        }
      }
    }
  ...
  //i l'ultima direcció feta per arribar-hi
  pair<int,Dir> give_help(Pos p, int imin, int imax, int jmin, int jmax) {
    bool found = false;
    pair<int,Dir> resultat;
    resultat.first = INF;
    resultat.second = Left;
    ...
    while (not Q.empty() and not found) {
      ...
      for (int i = 0; not found and i < 4; ++i) {
        Pos u = v + dirs_mag[i];
        if (pos_ok(u) and u.i >= imin and u.i <= imax and u.j >= jmin and u.j <= jmax and cell(u).type != Wall and distances[u.i][u.j] == INF) {
          Q.push(u);
          distances[u.i][u.j] = distances[v.i][v.j] + 1;
          previ[u.i][u.j] = dirs_mag[i];
  ...
  //i l'ultima direcció feta per arribar-hi
  pair<int,Dir> found_book(Pos p, int imin, int imax, int jmin, int jmax) {
    bool found = false;
    pair<int,Dir> resultat;
    resultat.first = INF;
    resultat.second = Left;
    ...
    while (not Q.empty() and not found) {
      ...
      for (int i = 0; not found and i < 4; ++i) {
        Pos u = v + dirs_mag[i];
        if (pos_ok(u) and u.i >= imin and u.i <= imax and u.j >= jmin and u.j <= jmax and cell(u).id == -1 and cell(u).id == -1 and cell(u).type != Wall and distances[u.i][u.j] == INF) {
          Q.push(u);
          distances[u.i][u.j] = distances[v.i][v.j] + 1;
          previ[u.i][u.j] = dirs_mag[i];
          if (cell(u).book) {
  ...
  //Aquesta funció retorna la distància mínima a un enemic
  //i l'ultima direcció feta per arribar-hi
  pair<int,Dir> found_enemy(Pos p, int imin, int imax, int jmin, int jmax) {
    bool found = false;
    pair<int,Dir> resultat;
    resultat.first = INF;
    resultat.second = Up;
    ...
    while (not Q.empty() and not found) {
      ...
      for (int i = 0; not found and i < 4; ++i) {
        Pos u = v + dirs_mag[i];
        if (pos_ok(u) and u.i >= imin and u.i <= imax and u.j >= jmin and u.j <= jmax and (cell(u).id == -1 or (cell(u).id != -1 and unit(cell(u).id).player != me())) and cell(u).type != Wall and distances[u.i][u.j] == INF) {
          Q.push(u);
          distances[u.i][u.j] = distances[v.i][v.j] + 1;
          previ[u.i][u.j] = dirs_mag[i];
          if (cell(u).owner != me() and cell(u).owner != -1 and cell(u).id != -1 and u != pos_voldemort() and unit(cell(u).id).type != Ghost) {
            found = true;
            resultat.first = distances[u.i][u.j];
            resultat.second = cami_invers_m(p, u, previ);
          }
  ...
  virtual void play () {
    ...
    for (int id_wiz : ids_mags) {
      ...
      if (unit(id_wiz).is_in_conversion_process()) { //que vagi a per ajuda
        ...
        if (round() < num_rounds() - 65) { //mode recolecta
          if (unit(id_wiz).pos.i < 15 or unit(id_wiz).pos.i > 45 or unit(id_wiz).pos.j < 15 or unit(id_wiz).pos.j > 45) { //volem estar en els quadrants 1 i 3 sempre
            Pos pos = look_no_wall(Pos(29,29));
            pair<int,Dir> cami0 = go_to(unit(id_wiz).pos, pos);
            Pos new_pos = unit(id_wiz).pos + cami0.second;
            if (pos_ok(new_pos) and cell(new_pos).type != Wall) move(id_wiz, cami0.second);
          }

          else {
            pair<int,Dir> cami0 = give_help(unit(id_wiz).pos, 15, 45, 15, 45);
            pair<int,Dir> cami1 = found_book(unit(id_wiz).pos, 15, 45, 15, 45);
            pair<int,Dir> cami2 = found_enemy(unit(id_wiz).pos, 15, 45, 15, 45);
          ...
          pair<int,Dir> cami0 = give_help(unit(id_wiz).pos, 0, 59, 0, 59);
          pair<int,Dir> cami1 = found_enemy(unit(id_wiz).pos, 0, 59, 0, 59);
