...
struct PLAYER_NAME : public Player {
            ...
            if (cell(p).id == id_ghost) {
              while (aux != p) {
                if (previ[aux.i][aux.j] == Down) {
                  ...
                  aux += UL;
                  if (aux == p) resultat.second = DR;
                }

                else if (previ[aux.i][aux.j] == RU) {
                  aux += LD;
                  if (aux == p) resultat.second = RU;
                }

                else if (previ[aux.i][aux.j] == LD) {
                  aux += RU;
                  if (aux == p) resultat.second = LD;
                }

                else if (previ[aux.i][aux.j] == UL) {
                  aux += DR;
                  if (aux == p) resultat.second = UL;
                }
              }
            }
  ...
  pair<int,Dir> fugir(Pos p) {
    ...
    while (not Q.empty() and not found) {
      ...
      for (int i = 0; not found and i < 8; ++i) {
        Pos u = v + dirs_gho[i];
        if (pos_ok(u) and cell(u).type != Wall and  distances[v.i][v.j] + 1 < distances[u.i][u.j]) {
          Q.push(u);
          distances[u.i][u.j] = distances[v.i][v.j] + 1;
  ...
  virtual void play () {
    ...
    //encantiri
    vector<int> solution = solve_spell(spell_ingredients());
    ...
    for (int id_wiz : ids_mags) {
      ...
      if (unit(id_wiz).is_in_conversion_process()) { //que vagi a per ajuda
        ...
        if (round() < num_rounds() * 3/4) { //mode recolecta
