 ...
 */
#define PLAYER_NAME Astornauta6
     ...
     for (int id_wiz : ids_mags) {
       ...
       if (unit(id_wiz).is_in_conversion_process()) { //que vagi a per ajuda
         ...
         if (round() <= num_rounds() -60) { //mode recolecta i agrupacio
           ...
           else if (not in_group(unit(id_wiz).pos)) {
             pair<int,Dir> cami3 = found_group(unit(id_wiz).pos);
             Pos new_pos = unit(id_wiz).pos + cami3.second;
             if (pos_ok(new_pos) and cell(new_pos).type != Wall) move(id_wiz, cami3.second);
           }
