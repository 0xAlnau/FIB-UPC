...
{	
    ...
    vec3 FocusSCO = vec3(0.0, 0.0, 0.0); //la càmera es el focus!!!

    vec3 L = FocusSCO - VertexSCO; // Focus - Vertex !!!!
    L = normalize(L);

    fcolor = Ambient() + Difus(NormSCO, L, colorFocus) + Especular(NormSCO, L, SCO_vertex, colorFocus); //matdiff no cal, es té en compte després
