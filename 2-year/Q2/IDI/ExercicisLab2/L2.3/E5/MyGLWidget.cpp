...
void MyGLWidget::initializeGL ()
{
  ...
  calculCentreRadi(glm::vec3(-2.5, 0.0, -2.5), glm::vec3(2.5, 1.0, 2.5)); //important fer-ho abans!
  projectTransform(); // HEM de fer el canvi!!! PM
  viewTransform(); //VM
  //animacio
  connect(&timer, SIGNAL (timeout()), this, SLOT (animar ()));
  timer.start(16); // s’activa cada 16 milisegons (60 cops per segon)
  //timer.stop(); // atura animació, pero no volem aturar-la
}

void MyGLWidget::animar()
{
    makeCurrent();
    anglePatGir += 1.0; // exemple del que es vol animar
    update();
}
...
void MyGLWidget::mouseMoveEvent(QMouseEvent* event)
{
    ...
    if ( event->buttons() == Qt::LeftButton && !( event->modifiers() & (Qt::ShiftModifier | Qt::AltModifier | Qt::ControlModifier ) ) ) { // controla que s’ha premut botó esquerre i cap modificador
        angle_trident += 1.0;
    }

    if ( event->buttons() == Qt::RightButton && !( event->modifiers() & (Qt::ShiftModifier | Qt::AltModifier | Qt::ControlModifier ) ) ) { // controla que s’ha premut botó esquerre i cap modificador
        angle_0 += 1.0;
    }
...
void MyGLWidget::calculCentreRadi(glm::vec3 punt_minim, glm::vec3 punt_maxim) {
    centre.x = (punt_maxim.x + punt_minim.x) / 2.0f;
    centre.y = (punt_maxim.y + punt_minim.y) / 2.0f;
    centre.z = (punt_maxim.z + punt_minim.z) / 2.0f;
...
//  glViewport (0, 0, ample, alt);
  ...
  paintPatricio(1, glm::vec3(2.0f, 0.0f, 2.0f), 0.0f);
  paintPatricio(1, glm::vec3(0.0f, 0.0f ,0.0f), 90.0f);
  paintPatricio(1, glm::vec3(-2.0f, 0.0f, -2.0f), 180.0f);
  nPat = 4.0;
  paintPatricio(1, glm::vec3(1.5f, 0.0f, 0.0f), 0.0f);
  nPat = 0.0;
...
void MyGLWidget::paintPatricio(float altura, glm::vec3 pos, float angle) {
    modelTransformPatricio(altura, pos, angle);
...
void MyGLWidget::creaBuffersTerra() {
    ...
    for (unsigned int i = 0; i < 6; ++i) {
        ...
        Tmax = glm::max(Tmax, Vertices[i]);
        Tmin = glm::min(Tmin, Vertices[i]);
    }
...
void MyGLWidget::modelTransformPatricio(float altura, glm::vec3 pos, float angle) {
    float escala = altura / (Pmax.x - Pmin.y);
    glm::vec3 cbase = glm::vec3((Pmax.x+Pmin.x)/2, Pmin.y, (Pmax.z+Pmin.z)/2);
    glm::mat4 TG (1.0);

    //rotacio en cercles
    TG = glm::translate(TG, glm::vec3(0.0, 0.0, pos.z));
    if (nPat == 4.0) TG = glm::rotate(TG, glm::radians(anglePatGir), glm::vec3(0.0, 1.0, 0.0));
    TG = glm::translate(TG, glm::vec3(pos.x, 0.0, 0.0));
    if (nPat == 4.0) TG = glm::rotate(TG, -glm::radians(anglePatGir), glm::vec3(0.0, 1.0, 0.0));
...
void MyGLWidget::modelTransformTerra() {
    //float escala = altura / (Pmax.x - Pmin.y);
    glm::vec3 cbase = glm::vec3((Tmax.x+Tmin.x)/2, Tmin.y, (Tmax.z+Tmin.z)/2);
    glm::mat4 TG (1.0);
