...
void MyGLWidget::paintTerra() {
    ...
    glUniform3f(objectColor, COLOR_TERRA.x, COLOR_TERRA.y, COLOR_TERRA.z); //E6 canviar color
    glUniform1f(terra, 1.0f);
...
void MyGLWidget::carregaShaders() { // declarem-lo també en MyGLWidget.h
    ...
    objectColor = glGetUniformLocation (program->programId(), "objectColor");
    terra = glGetUniformLocation (program->programId(), "terra");
    changed = glGetUniformLocation (program->programId(), "changed");
}
...
void MyGLWidget::canvia_color(int v1, int v2, int v3) {
    makeCurrent(); //IMPORTANT E6 canviar color

    COLOR_TERRA.x = float(v1) / 255.0f;
    COLOR_TERRA.y = float(v2) / 255.0f;
    COLOR_TERRA.z = float(v3) / 255.0f;

    glUniform1f(changed, 1.0f);
