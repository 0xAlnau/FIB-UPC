...
void MyGLWidget::initializeGL ()
{
  ...
  emit envia_R(0.5 * 255);
  emit envia_G(0.7 * 255);
  emit envia_B(1.0 * 255);
...
void MyGLWidget::mouseMoveEvent(QMouseEvent *e)
{
    ...
    emit envia_psi(int(psi));
    emit envia_theta(int(theta));
...
// useu els paràmetres que considereu (els que hi ha són els de per defecte)
  glViewport (0, 0, ample * percent_ample, alt * percent_alt);
...
void MyGLWidget::creaBuffersTerra() {
    ...
    Colors[0] = COLOR_TERRA;
    Colors[1] = COLOR_TERRA;
    Colors[2] = COLOR_TERRA;
    Colors[3] = COLOR_TERRA;
    Colors[4] = COLOR_TERRA;
    Colors[5] = COLOR_TERRA;
...
void MyGLWidget::viewTransform() { //VM amb euler, CORRECTE
    ...
    View = glm::rotate(View, glm::radians(theta), glm::vec3(1.0, 0.0, 0.0)); //0 theta
    View = glm::rotate(View, glm::radians(psi), glm::vec3(0.0, 1.0, 0.0)); //psi trident
...
void MyGLWidget::canvia_psi(int valor) {
    makeCurrent(); //important per veure com canvia!

    if (valor > psi_anterior) {
        psi += 1.0f;
    }
    else {
        psi -= 1.0f;
...
void MyGLWidget::canvia_theta(int valor) {
    makeCurrent(); //important per veure com canvia!!

    if (valor > theta_anterior) {
        theta += 1.0f;
    }
    else {
        theta -= 1.0f;
    ...
    theta_anterior = valor;
    viewTransform();
    update();
}

void MyGLWidget::canvia_color(int v1, int v2, int v3) {
    makeCurrent(); //IMPORTANT E7 canviar color

    COLOR_FONS.x = float(v1) / 255.0f;
    COLOR_FONS.y = float(v2) / 255.0f;
    COLOR_FONS.z = float(v3) / 255.0f;

    glClearColor(COLOR_FONS.x, COLOR_FONS.y, COLOR_FONS.z, 1.0); // defineix color de fons

    update();
}


void MyGLWidget::canvia_alt(int valor) {
    percent_alt = float(valor) / 100.0f;
    update();

}

void MyGLWidget::canvia_ample(int valor) {
    percent_ample = float(valor) / 100.0f;
    update();
}
