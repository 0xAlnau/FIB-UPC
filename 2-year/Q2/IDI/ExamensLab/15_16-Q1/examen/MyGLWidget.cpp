...
void MyGLWidget::calculaParEscena() {
  centreEsc.x = (EscMax.x + EscMin.x) / 2;
  centreEsc.y = (EscMax.y + EscMin.y) / 2;
  centreEsc.z = (EscMax.z + EscMin.z) / 2;

  radi = glm::distance(EscMax, EscMin) / 2;
  d = 2 * radi;
}
...
void MyGLWidget::paintGL ()
{
  ...
  modelTransformPatricio (glm::vec3(0.0, 0.0, zetes), 3.0, 0.0);
  glDrawArrays(GL_TRIANGLES, 0, patr.faces().size()*3);

  // Activem el VAO per a pintar el Patricio 2
  glBindVertexArray (VAO_Patr);
  modelTransformPatricio (glm::vec3(0.0, 3.0, zetes), 2.5, 180.f);
  glDrawArrays(GL_TRIANGLES, 0, patr.faces().size()*3);

  // Activem el VAO per a pintar el Patricio 3
  glBindVertexArray (VAO_Patr);
  modelTransformPatricio (glm::vec3(0.0, 5.5, zetes), 2.0, 0.0);
  glDrawArrays(GL_TRIANGLES, 0, patr.faces().size()*3);
...
void MyGLWidget::resizeGL (int w, int h)
{
  ...
  if (rav != 1.0f) {
    ...
    if (rav > 1.0f) {
      ...
      float alphav = glm::atan(glm::tan(radi/d) / rav);
      float FOVnou = 2 * alphav;
...
void MyGLWidget::createBuffers ()
{
  ...
  glm::vec3 diff(0.8,0.8,0.0); // color
  glm::vec3 spec(1.,1.,1.);
  float shin = 1000;
...
void MyGLWidget::loadShaders ()
{
  ...
  varFranges = glGetUniformLocation (program->programId(), "varFranges");
}

void MyGLWidget::modelTransformPatricio (glm::vec3 pos, float size, float angle)
{
  float escala = size / (Pmaxy - Pminy);
...
{
  glm::mat4 Proj;  // Matriu de projecció
  Proj = glm::perspective(2 * glm::asin(radi/d), 1.0f, d-radi, d+radi);
  glUniformMatrix4fv (projLoc, 1, GL_FALSE, &Proj[0][0]);
}
...
void MyGLWidget::keyPressEvent (QKeyEvent *e)
{
  switch (e->key())
  {
    ...
    case Qt::Key_L:
        if (colF.y == 0.0) colF = glm::vec3(1.0, 1.0, 1.0);
        else colF = glm::vec3(1.0, 0.0, 1.0);
        glUniform3fv(colFocus, 1, &colF[0]);
        break;

    case Qt::Key_X:
        franges = !franges;
        glUniform1f(varFranges, franges);
        break;
