...
void MyGLWidget::calculaParamEsc() {
...
void MyGLWidget::createBuffers()
{
  ...
  calculaCapsaModelLego ();

  // Creació del Vertex Array Object del Legoicio
...
void MyGLWidget::loadShaders ()
{
  ...
  varBlau = glGetUniformLocation (program->programId(), "blau");
  glUniform1f(varBlau, blau);
  varFocus = glGetUniformLocation(program->programId(), "posFocus");
  glUniform3fv(varFocus, 1, &posFocus[0]);
}

void MyGLWidget::modelTransformPatricio ()
{
  glm::mat4 TG(1.f);  // Matriu de transformació
  float escala = 3.0 / (PatMaxy - PatMiny);

  TG = glm::translate(TG, glm::vec3(10.0, 0.0, -10.0));
  TG = glm::rotate(TG, glm::radians(-45.f), glm::vec3(0.0, 1.0, 0.0));
...
{
  glm::mat4 TG(1.f);  // Matriu de transformació
  float escala = 3.0 / (LegoMaxy - LegoMiny);

  TG = glm::translate(TG, glm::vec3(0.0, 0.0, 0.0));
  TG = glm::rotate(TG, glm::radians(135.f) + angle, glm::vec3(0.0, 1.0, 0.0));
  TG = glm::scale(TG, glm::vec3(escala, escala, escala));
...
void MyGLWidget::calculaCapsaModelPat()
{
...
void MyGLWidget::calculaCapsaModelLego()
{
  // Càlcul capsa contenidora i valors transformacions inicials
  float minx, miny, minz, maxx, maxy, maxz;
...
void MyGLWidget::keyPressEvent (QKeyEvent *e)
{
  switch (e->key())
  {
    ...
    case Qt::Key_L:
        if (posFocus.x == 0.0) posFocus = glm::vec3(10.0, 10.0, 10.0);
