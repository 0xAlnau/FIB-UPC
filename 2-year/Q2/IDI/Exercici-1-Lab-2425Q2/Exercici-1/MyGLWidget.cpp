...
void MyGLWidget::pintaSector(){
    for (float i = 0.0; i < 16.0; ++i) {
        modelTransformSector(glm::radians(i * angleSectorNoria + angleGirar));
        glBindVertexArray(VAO_Sector);
        glUniform1f(num_referencia, 1.0); //anem canviant el numero de referencia, sector = 1
...
void MyGLWidget::pintaBase(){
    ...
    glUniform1f(num_referencia, 2.0); //anem canviant el numero de referencia, base = 2
    glDrawArrays(GL_TRIANGLES, 0, 3);
}

void MyGLWidget::pintaCistella(){
    if (VuitCistelles == 0.0) { //volem una cistella
        modelTransformCistella(glm::radians(angleGirar));
        glBindVertexArray(VAO_Cistella);
        glUniform1f(num_referencia, 3.0); //anem canviant el numero de referencia, cistella = 3
        glDrawArrays(GL_TRIANGLES, 0, 18);
    }
    else {
        for (float i = 0.0; i < 8.0; ++i) {
            modelTransformCistella(glm::radians(i * angleCistellaNoria + angleGirar));
            glBindVertexArray(VAO_Cistella);
            glUniform1f(num_referencia, 3.0); //anem canviant el numero de referencia, cistella = 3
            glDrawArrays(GL_TRIANGLES, 0, 18);
        }
...
void MyGLWidget::modelTransformCistella(float angleRadians){
    glm::mat4 TG (1.0f);
    TG = glm::translate(TG, glm::vec3(0.0, -0.25, 0.0)); //movem 0.25 cap avall
    TG = glm::rotate(TG, angleRadians, glm::vec3(0.0, 0.0, 1.0));
    TG = glm::translate(TG, glm::vec3(0.5, 0.0, 0.0)); //movem 0.5 a la dreta
    TG = glm::rotate(TG, -angleRadians, glm::vec3(0.0, 0.0, 1.0));
    TG = glm::scale(TG, glm::vec3(1.0/10.0));
    glUniformMatrix4fv(TGLoc, 1, GL_FALSE, &TG[0][0]);
}

void MyGLWidget::modelTransformBase(){
    glm::mat4 TG (1.0f);
    TG = glm::translate(TG, glm::vec3(0.0, -0.656, 0.0));
    TG = glm::scale(TG, glm::vec3(1.0, 0.40, 1.0));
    glUniformMatrix4fv(TGLoc, 1, GL_FALSE, &TG[0][0]);
}
...
void MyGLWidget::keyPressEvent(QKeyEvent* event)
{
    ...
    switch (event->key()) {
        ...
        case Qt::Key_Left: {
            angleGirar += 5.0; //sumem 5 graus
            break;
        }
        case Qt::Key_Right: {
            angleGirar -= 5.0; //restem 5 graus
            break;
        }
        case Qt::Key_1: {
            VuitCistelles = 0.0; //false, una cistella volem
            break;
        }
        case Qt::Key_8: {
            VuitCistelles = 1.0; //true, 8 cistelles volem
            break;
        }
        default: event->ignore(); break; //restem 5 graus
    }
    update();
}
...
void MyGLWidget::carregaShaders()
{
  ...
  num_referencia = glGetUniformLocation(program->programId(), "num_referencia");
}
