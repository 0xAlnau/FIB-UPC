...
public:
  ...
  class iterator {
    ...
    /* Pre: el p.i apunta a un element que no és ni l'end ni el begin de la llista.
     * Post: el p.i seguirà apuntat al mateix element, però aquest element s'haurà mogut
     * un posició cap a la dreta de la llista. Si el p.i apunta a l'últim element de la
     * llista no canvia res.*/
    void moveTowardsEnd() {
      if (pitem->next == &(plist->itemsup)) {}
      else {
        Item* element = pitem;
        Item* next_element = pitem->next;

        element->prev->next = next_element;
        next_element->prev = element->prev;

        next_element->next->prev = element;
    ...
    /* Pre: el p.i apunta a un element que no és ni l'end ni el begin de la llista.
     * Post: el p.i apuntarà al mateix element, però aquest s'ha desplaçat una posició a
     * l'esquerra de la llista. Si el p.i apunta al primer element de la llista no canvia res.*/
    void moveTowardsBegin(){
      if (pitem->prev == &(plist->iteminf)) {}
      else {
        Item* element = pitem;
        Item* prev_element = pitem->prev;

        element->next->prev = prev_element;
        prev_element->next = element->next;

        prev_element->prev->next = element;
        element->prev = prev_element->prev;
       ...
       el resultat és el p.i. */
    {
      ...
      else if (hook_active) (*this).moveTowardsEnd(); //movem una posició cap endavant
       ...
       el resultat és el p.i. */
    {
      ...
      else if (hook_active) (*this).moveTowardsBegin(); //movem una posició cap enrere
