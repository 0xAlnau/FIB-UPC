...
public:
  ...
  class iterator {
    ...
    iterator() {
        rebot = false; // si es false fa el comportament normal d'un iterador
    }
       ...
       el resultat és el p.i. */
    {
      if (plist->_size == 0) {}
        ...
        if (pitem == &(plist->itemsup) and not rebot) rebot = true; //canvi de sentit
        else if (pitem == plist->iteminf.next and rebot) rebot = false;
        else if (rebot) pitem = pitem->prev;
        else if (not rebot) pitem = pitem->next;
      }
      return *this;
    }
       ...
       el resultat és el p.i. */
    {
      if (plist->_size == 0) {}
        ...
        if (pitem == plist->iteminf.next and not rebot) rebot = true;
        else if (pitem == &(plist->itemsup) and rebot) rebot = false;
        else if (rebot) pitem = pitem->next;
        else if (not rebot) pitem = pitem->prev;
      }
      return *this;
    }
