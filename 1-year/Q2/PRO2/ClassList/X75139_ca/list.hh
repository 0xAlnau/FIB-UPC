...
public:
  ...
  void moveTowardsEnd(iterator &it) {
    if (it.pitem->next == &itemsup or _size == 1) {}
    else {
      Item* element = it.pitem;
      Item* element_nex = it.pitem->next;

      element->prev->next = element_nex;
      element_nex->prev = element->prev;

      element_nex->next->prev = element;
      element->next = element_nex->next;

      element->prev = element_nex;
      element_nex->next = element;
