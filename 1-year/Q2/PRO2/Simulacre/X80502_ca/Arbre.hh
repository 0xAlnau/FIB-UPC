...
Arbre suma (const Arbre& a) const{
  Arbre resultat;
  if (primer_node == NULL and a.primer_node != NULL) {}

  else if (primer_node != NULL and a.primer_node == NULL) {}

  else if (primer_node == NULL and a.primer_node == NULL) {}

  else { //cap és nul
    node_arbre* arrel = new node_arbre;
    arrel->info = primer_node->info + a.primer_node->info;

    node_arbre* left = sum_aux(primer_node->segE, a.primer_node->segE);
    node_arbre* right = sum_aux(primer_node->segD, a.primer_node->segD);

    arrel->segE = left;
    arrel->segD = right;

    resultat.primer_node = arrel;
...
static node_arbre* sum_aux(node_arbre* n,  node_arbre *m){
  node_arbre* resultat = NULL;

  if (n == NULL and m != NULL) {}

  else if (n != NULL and m == NULL) {}
    ...
    node_arbre* left = sum_aux(n->segE, m->segE);
    node_arbre* right = sum_aux(n->segD, m->segD);
